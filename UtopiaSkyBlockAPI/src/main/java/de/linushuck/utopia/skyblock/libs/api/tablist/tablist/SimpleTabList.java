package de.linushuck.utopia.skyblock.libs.api.tablist.tablist;
...
import de.linushuck.utopia.skyblock.libs.api.tablist.item.TabItem;
import de.linushuck.utopia.skyblock.libs.api.tablist.util.Packets;
...
public class SimpleTabList extends TitledTabList implements CustomTabList
{
    ...
    private static final Set<PlayerInfoAction> ADD_PLAYER_ACTIONS = new HashSet<>(Arrays.asList(PlayerInfoAction.ADD_PLAYER, PlayerInfoAction.UPDATE_LISTED));
    ...
    private List<PacketContainer> getUpdate(Map<Integer, TabItem> oldItems, Map<Integer, TabItem> newItems)
    {
        ...
        for(Entry<Integer, TabItem> entry : newItems.entrySet())
        {
            ...
            { // TabItem has been removed.
                removePlayer.add(getPlayerInfoData(index, oldItem).getProfileId());
                continue;
            }

            boolean skinChanged = oldItem == null || newItem.updateSkin() || !newItem.getSkin()
                    .equals(oldItem.getSkin());
            boolean textChanged = oldItem == null || newItem.updateText() || !newItem.getText()
                    .equals(oldItem.getText());
        ...
        {
            result.add(Packets.getRemovePacket(removePlayer));
            result.add(Packets.getPacket(ADD_PLAYER_ACTIONS, addPlayer));
        }
    ...
    private WrappedGameProfile getGameProfile(int index, TabItem item)
    {
        ...
        { // Profile is not cached, generate and cache one.
            ...
            profile.getProperties().put(Skin.TEXTURE_KEY, item.getSkin().property());
